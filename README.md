...
`roc_desk-explorer` 是 roc_desk 多仓库拆分后的独立工具仓库。

## 用途

提供本地和远程文件的双栏浏览、目录树、多选、复制移动、重命名、删除和文件预览。

## 依赖

- 公共基础库：`roc_desk-common` 的 `roc_desk_core`，固定使用对应 tag。
- 工具专属依赖：roc_desk-common core/common；FileOps 文件访问契约；SSH/SFTP 或 Agent 适配器。
...
默认生成 Release 版本：`bin\roc_desk-explorer.exe`。开发构建使用：
...
```markdown
![资源管理器主界面](docs/screenshots/main.png)
```
